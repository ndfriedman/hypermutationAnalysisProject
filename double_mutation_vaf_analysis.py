...
def compare_vafs(maf):
    ...
    for case in cases:
        ...
        diffsAllMuts = analysis_utils.calculate_all_pairwise_differences(np.array(list(caseMafAllMuts['ccf_Mcopies'])))
        diffsOncogenicMuts = analysis_utils.calculate_all_pairwise_differences(np.array(list(caseMafAllOncogenicMuts['ccf_Mcopies'])))
        print np.nanmean(diffsAllMuts), np.nanmean(diffsOncogenicMuts)
...
cohortMaf = mafAnnoMafWithData[mafAnnoMafWithData['Tumor_Sample_Barcode'].isin(colorectalHyperIds)]

compare_vafs(cohortMaf)

cohortMaf.columns.values
